...
가위바위보 게임의 목표은 언제나 '나'가 승리할 수 있도록 물음표 카드에 들어갈 카드를 뽑는 것이다. 이를 통해 플레이어의 인능력을 평가한다.
...
5. 반대로, '나'에게 '물음표' 카드가 주어지고, '상대'에게 '가위' 카드가 주어졋다면, 플레이어는 '바위' 카드를 선택해서 '나'가 승리할 수 있도록 해야 한다.
...
2라운드에서는 조건들을 결정하고, 해당 조건들에 맞는 정답을 배열로 가지고 있는다. 그리고 버튼의 순서를 뒤섞은 임시 배열을 가지고 있다고, 버튼을 활성상태(enable)로 바꾸면서 뒤섞인 배열의 버튼으로 교체한다. 동시에 플레이어에게 조건을 제시해준다. 플레이어의 클릭 순서를 입력으로 받으면서 시간을 기록하고, 정답 배열과 달라지는 순간 실패처리한다. 모든 입력이 정답 배열과 같으면 정답처리 후 다음 문제로 넘어간다.
