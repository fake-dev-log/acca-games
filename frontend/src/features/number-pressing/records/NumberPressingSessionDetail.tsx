...
export function NumberPressingSessionDetail() {
  ...
  if (error) {
    return <RecordPageLayout backPath={`/records/${GameCodeSlugs.NUMBER_PRESSING}`} title="세션 상세 기록" sidebarContent={sidebar}><p className="text-red-500">{error}</p></RecordPageLayout>;
  }
  if (!currentSession) {
  ...
  const totalTrialsR1 = resultsR1.length;
  const correctTrialsR1 = resultsR1.filter(r => r.isCorrect).length;
  const totalTrialsR2 = resultsR2.length;
  const correctTrialsR2 = resultsR2.filter(r => r.isCorrect).length;
  ...
  const avgResponseTimeR1 = totalTrialsR1 > 0 ? (resultsR1.reduce((sum, r) => sum + r.timeTaken, 0) / totalTrialsR1) : 0;
  const avgResponseTimeR2 = totalTrialsR2 > 0 ? (resultsR2.reduce((sum, r) => sum + r.timeTaken, 0) / totalTrialsR2) : 0;
