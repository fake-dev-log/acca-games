...
export function RpsRecords() {
  ...
  const handleSessionClick = (sessionId: number) => {
    navigate(`/records/${GameCodeSlugs.RPS}/${sessionId}`);
  };

  const calculateSessionAccuracy = (session: types.RpsSessionWithResults) => {
  ...
  return (
    <RecordPageLayout backPath="/records" title="가위바위보 게임 기록" sidebarContent={<SessionList sessions={sessionsForList} loading={loading} error={error} onSessionClick={handleSessionClick} />}>
