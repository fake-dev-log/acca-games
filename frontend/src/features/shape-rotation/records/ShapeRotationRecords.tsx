...
export function ShapeRotationRecords() {
  ...
  const { paginatedSessions, loading, error, fetchPaginatedSessions } = useShapeRotationStore();
  ...
  const handleSessionClick = (sessionId: number) => {
    navigate(`/records/${GameCodeSlugs.SHAPE_ROTATION}/${sessionId}`);
  };

  const calculateSessionAccuracy = (session: types.ShapeRotationSessionWithResults) => {
  ...
  return (
    <RecordPageLayout backPath="/records" title="도형 회전하기 게임 기록" sidebarContent={<SessionList sessions={sessionsForList} loading={loading} error={error} onSessionClick={handleSessionClick} />}>
